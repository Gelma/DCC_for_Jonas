 ...
 *
 * Rhyolite Software DCC 1.3.158-1.330 $Revision$
 */
...
find_srvr_rcd_type(DCC_SRVR_ID tgt_id)
{
	...
	return find_srvr_rcd(&srvr_id_sum, DCC_ID_INVALID, 0,
			     "checking server-ID state");
}
		 ...
		 const char *str)
{
	...
	i = find_srvr_rcd(sum, DCC_ID_INVALID, 0, str);
	if (i < 0)
		return;			/* broken database */
	if (i > 0
	    && old != 0
	    && DB_RCD_ID(db_sts.rcd2.d.r) == id
	    && ts2secs(&db_sts.rcd2.d.r->ts) > old)
		return;
	      ...
	      DB_STATE *rcd_st)		/* put the record or garbage here */
{
	...
	switch (db_lookup(dcc_emsg, DCC_CK_SRVR_ID, &sum, hash_st, rcd_st, 0)) {
	case DB_FOUND_SYSERR:
		return -1;
	case DB_FOUND_IT:
		return 1;
 ...
 *	use only db_sts.hash and db_sts.rcd2
 *	leave the result in db_sts.rcd2 */
	    ...
	    u_char must_have_it)	/* 1=database broken if cksum absent */
{
	...
	if (!pfound_ck)
		pfound_ck = &found_ck;
	switch (db_lookup(dcc_emsg, type, sum,
			  &db_sts.hash, &db_sts.rcd2, pfound_ck)) {
	case DB_FOUND_SYSERR:
		DB_ERROR_MSG(dcc_emsg);
...
/* compute summarizable total for one checksum starting at db_sts.rcd2
 *	changes db_sts.hash and db_sts.rcd2 */
...
#	define STS db_sts.rcd2
	...
	rcd_tgts = get_ck_tgts(&oldest_ck, type, sum, must_have_it);
	...
	for (;;) {
		...
		if (DB_RCD_SUMRY(STS.d.r))
			break;

		/* honor deletions */
		rcd_tgts = DB_TGTS_RCD(STS.d.r);
		if (rcd_tgts == 0)
			break;
		 ...
		 * inflating totals. */
		if (DB_RCD_ID(STS.d.r) == my_srvr_id) {
		...
		if (summarize_limit_secs < db_time.tv_sec - (24*60*60+60*60)) {
			summarize_limit_secs = (db_time.tv_sec
						- DCC_OLD_SPAM_SECS);
#if 24*60*60+60*60 > DATE_RCD_RANGE_SECS
#error "date rcd range too small"
#endif
			i = find_date_rcd(summarize_limit_secs,
					  &db_sts.hash, &STS);
			if (i < 0) {
				DB_ERROR_MSG(dcc_emsg);
				return DCC_TGTS_INVALID;
			...
			if (i > 0)
				summarize_limit = STS.s.rptr;
		}
		if (prev <= summarize_limit)
			break;

		oldest_ck = db_map_rcd_ck(dcc_emsg, &STS, prev, type);
		if (!oldest_ck) {
			DB_ERROR_MSG(dcc_emsg);
			return DCC_TGTS_INVALID;
		}
 ...
 *	On entry db_sts.sumrcd points to the record whose checksums are
...
summarize_rcd(u_char dly)		/* 1=working on delayed records */
{
	...
	if (move_ok) {
		...
		memcpy(&new, db_sts.sumrcd.d.r, DB_RCD_LEN(db_sts.sumrcd.d.r));
 ...
 *	The record of checksum types is specified by db_sts.sumrcd.
 *	    It might not be the leader for any of its checksums.
 *	db_sts.sumrcd might be changed.
 *	Use db_sts.rcd, db_sts.hash, db_sts.rcd2, db_sts.free, db_sts.tmp */
...
summarize_dly(void)
{
	...
	do {
		...
		ck_tgts = get_ck_tgts(0, type, &cur_ck->sum, 1);
	   ...
	   const DCC_CK_TYPES type)
{
	...
	for (limit = 20; limit >= 0; --limit) {
		...
		if (!DB_CK_JUNK(ck)
		    && DB_RCD_ID(db_sts.rcd2.d.r) != DCC_ID_COMP)
			return 1;
	}
	...
	*max_tgts = 0;
	...
	for (ck = q->pkt.r.cks; ck < ck_lim; ++ck) {
		...
		if (num_rcd_cks > 0
			...
			p_tgts = get_ck_tgts(0, type, &ck->sum, 0);
	...
	case DB_FOUND_IT:
		...
		for (;;) {
			...
			for (i = DB_NUM_CKS(db_sts.rcd.d.r);
				 ...
				 * the embargo is over. */
				if (ts_newer_ts(&old_ts, &db_sts.rcd.d.r->ts))
					return DCC_TGTS_OK;
...
do_delete(QUEUE *q, DCC_CLNT_ID id)
{
	...
	if (delete_sub(q, &q->pkt.d.ck, 0)) {
		...
		if (!DCC_CK_IS_REP0(grey_on, q->pkt.d.ck.type)
