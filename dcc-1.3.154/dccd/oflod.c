 ...
 *
 * Rhyolite Software DCC 1.3.154-1.187 $Revision$
 */
...
oflod_parse_map(FLOD_OPTS *opts, const char *str0, int lno)
{
	...
	if (opts->maps.len >= DIM(opts->maps.entry)) {
		dcc_error_msg("too many ID mappings with\"%s\"%s",
			      str0, dcc_fnm_lno(&fnm_buf, flod_path, lno));
		return 0;
	}
	...
	if (!CLITCMP(str0, "self->")) {
		...
		if (*str++ != '-' || *str++ != '>') {
			dcc_error_msg("invalid server-ID mapping \"%s\"%s",
				      str0,
				      dcc_fnm_lno(&fnm_buf, flod_path, lno));
			return 0;
		}
	}
	if (!strcasecmp(str, "self")) {
		...
		dcc_error_msg("invalid ID mapping result \"%s\"%s",
			      str, dcc_fnm_lno(&fnm_buf, flod_path, lno));
		return 0;
	}
		...
		const char *buf, int lno)
{
	...
	while (*buf_ptr != '\0') {
		...
		dcc_error_msg("unknown option \"%s\"%s",
			      opt, dcc_fnm_lno(&fnm_buf, flod_path, lno));
		return 0;
	}
	...
	if (!f) {
		if (flod_mtime != 0) {
			dcc_error_msg("fopen(%s): %s",
				      DB_NM2PATH_ERR(flod_path), ERROR_STR());
			flod_mtime = 0;
		}
...
oflod_ck_put(void)
{
	...
	if (DB_RCD_ID(db_sts.rcd.d.r) == DCC_ID_WHITE
	    ...
	    || ts2secs(&db_sts.rcd.d.r->ts) < db_time.tv_sec - FLOD_STALE_SECS
	...
	if (flod_mmaps && !db_minimum_map) {
		...
		while (flod_mmaps->delay_pos < db_csize) {
			...
			if (DB_RCD_DELAY(db_sts.sumrcd.d.r)) {
				...
				if (ts_newer_ts(&db_sts.sumrcd.d.r->ts, &past))
					break;
				if (TMSG_BIT(DB))
				...
				if (TMSG_BIT(DB)) {
					...
					if (us >= DCC_US/10)
					    dcc_trace_msg("spent %.3f seconds"
							" summarizing "L_HxPAT,
							us/(DCC_US*1.0),
							flod_mmaps->delay_pos);
				}
