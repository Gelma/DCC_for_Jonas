 ...
 *
 * Rhyolite Software DCC 1.3.158-1.101 $Revision$
 */
...
#    define	 FLODMAP_FG_NEED_REWIND	0x00020000  /* database purged */
...
#define _ts_newer_ts(ts1, ts2) (memcmp(ts1, ts2, sizeof(DCC_TS)) > 0)
#ifdef HAVE_GCC_INLINE			/* no prototypes without inline */
static inline int
ts_newer_ts(const DCC_TS *ts1, const DCC_TS *ts2)
{ return _ts_newer_ts(ts1, ts2); }
#else
#define ts_newer_ts(ts1, ts2) (memcmp(ts1, ts2, sizeof(DCC_TS)) > 0)
#endif

#define _ts_older_ts(ts1, ts2) (memcmp(ts1, ts2, sizeof(DCC_TS)) < 0)
#ifdef HAVE_GCC_INLINE			/* no prototypes without inline */
static inline int
ts_older_ts(const DCC_TS *ts1, const DCC_TS *ts2)
{ return _ts_older_ts(ts1, ts2); }
#else
#define ts_older_ts(ts1, ts2) _ts_older_ts(ts1, ts2)
#endif
