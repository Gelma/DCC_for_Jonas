 ...
 *
 * Rhyolite Software DCC 1.3.158-1.124 $Revision$
 */
 ...
 *	- At helper.debug >=1, log the first list check in a group
...
time_ck_post(DNSBL_WORK *dlw,
	     DNSBL_HIT hit,
	     u_char timedout)		/* 1=DNS resolver timed out */
{
	struct timeval now;
	time_t used_us;
	...
	if (helper.debug > 1) {
		if (timedout)
			dnsbl_log(dlw, 0, hit,
				  "timed out after using %.1f url_secs",
				  (used_us - dlw->url_us_used) / (DCC_US*1.0));
		else if ((used_us - dlw->url_us_used) > url_us/4)
			dnsbl_log(dlw, 0, hit,
				  "finished after using %.1f url_secs",
				  (used_us - dlw->url_us_used) / (DCC_US*1.0));
	}
...
#	define NUM_SUSTRS 3
	...
	if (lists != 0 || helper.debug < 2) {
		dcc_host_unlock();
		return time_ck_post(dlw, src, 0);
	}
	  ...
	  DNSBL_HIT ns_src)		/* DNSBL_HIT_{MHOST,URL}_NS */
{
	...
	if (res_len < 0) {
		...
		return (time_ck_post(dlw, src, h_errno == TRY_AGAIN)
...
dnsbl_work(const DNSBL_REQ *req, DNSBL_RESP *resp)
{
	...
	for (gnum = 0, hg = dlw.hgroups; gnum < NUM_DNSBL_GROUPS; ++gnum, ++hg) {
	   ...
	   DNSBL_HIT src)		/* where name or address was found */
{
	...
	result = ask_helper(dlw->dcc_ctxt, dlw->log_ctxt, dlw->url_us,
			    &req.hdr, sizeof(req), &resp.hdr, sizeof(resp));
	time_ck_post(dlw, src, !result);
	msg_secs_fin(dlw);
	if (!result)
		return;
...
dcc_client_dnsbl(DNSBL_WORK *dlw, const struct in6_addr *addr, const char *name)
{
	...
	dnsbl_client(dlw);
	time_ck_post(dlw, DNSBL_HIT_CLIENT, 0);
	msg_secs_fin(dlw);
}
...
dcc_mail_host_dnsbl(DNSBL_WORK *dlw, const char *host)
{
	...
	time_ck_post(dlw, DNSBL_HIT_MHOST, 0);
	msg_secs_fin(dlw);
}
...
url_dnsbl(DNSBL_WORK *dlw, const char *dom)
{
	...
	time_ck_post(dlw, DNSBL_HIT_URL, 0);
	msg_secs_fin(dlw);
}
