 ...
 *
 * Rhyolite Software DCC 1.3.154-1.301 $Revision$
 */
	...
	if (loc.sa.sa_family != AF_UNSPEC
		...
		if (0 > bind(s, &loc.sa, DCC_SU_LEN(&loc)))
			dcc_error_msg("bind(flood %s%s): %s",
				      dcc_su2str_err(&loc),
				      dcc_fnm_lno(&fnm_buf, flod_path, ofp->lno),
				      ERROR_STR());
	}
}
...
typedef enum {
    ...
    CK_DUP_CONTINUE,			/* continue searching current chain */
...
static CK_DUP
ck_dup_rcd(IFLOD_INFO *ifp, DB_RCD *new)
{
 ...
 *	also changes db_sts.hash and db_sts.rcd2 */
static CK_DUP
ck_dup_ck_chain(IFLOD_INFO *ifp, DB_RCD *new,
...
#define MAX_CHAIN 100
	...
	for (cnt = 1, deleted = 0; ; ++cnt) {
		if (DB_TGTS_RCD_RAW(db_sts.rcd.d.r) == DCC_TGTS_DEL) {
			...
			if (new_tgts_raw != DCC_TGTS_DEL
			    && !ts_newer_ts(&new->ts, &db_sts.rcd.d.r->ts)) {
				reason = "deleted ";
		...
		/* If it is a duplicate report based on server-ID and timestamp,
		 * then save the copy that has the most checksums. */
		if (DB_RCD_ID(db_sts.rcd.d.r) == DB_RCD_ID(new)
		    && !memcmp(&new->ts, &db_sts.rcd.d.r->ts, sizeof(new->ts))) {
			result = ck_dup_rcd(ifp, new);
			break;
		}

		/* If count in the new report is larger than the existing total,
		 * then the new report cannot be a duplicate */
		 ...
		 * an existing, duplicate copy of the record.
		...
		if (cnt == 5) {
			...
			if (i > 0 && limit < db_sts.rcd2.s.rptr) {
				limit = db_sts.rcd2.s.rptr;
				limit_str = "date limit ";
			}
		}
		if (db_sts.rcd.s.rptr < limit) {
			reason = limit_str;
			result = CK_DUP_NEXT_CK;
			break;
		}
		...
		if (prev >= db_sts.rcd.s.rptr) {
			dcc_pemsg(EX_DATAERR, dcc_emsg,
				  "bad %s link of "L_HxPAT" at "L_HxPAT,
				  DB_TYPE2STR(type), db_sts.rcd.s.rptr, prev);
			iflod_close(ifp, 1, 1, 1, "%s", dcc_emsg);
			DB_ERROR_MSG(dcc_emsg);
			return CK_DUP_BROKEN;
		}

		found_ck = db_map_rcd_ck(dcc_emsg, &db_sts.rcd, prev, type);
		if (!found_ck) {
			iflod_close(ifp, 1, 1, 1, "%s", dcc_emsg);
			DB_ERROR_MSG(dcc_emsg);
			return CK_DUP_BROKEN;
	...
	if (((cnt >= MAX_CHAIN && TMSG_BIT(DB)) || TMSG_FB2(ifp->ofp))
		...
		const char *rstr = 0;
		switch (result) {
		case CK_DUP_BROKEN: rstr = "broken"; break;
		case CK_DUP_CONTINUE: rstr = "continue"; break;
	  ...
	  const DCC_FLOD_STREAM *stream, int max_len)
{
	...
	for (new_ck = new.cks; new_ck < &new.cks[DB_NUM_CKS(&new)]; ++new_ck) {
		...
		case DB_FOUND_IT:
			...
			r = ck_dup_ck_chain(ifp, &new, new_tgts, type, found_ck);
			switch (r) {
			case CK_DUP_BROKEN:
				return -1;
			case CK_DUP_CONTINUE:
				dcc_logbad(EX_SOFTWARE,
					   "impossible ck_dup_ck_chain()");
				break;
			case CK_DUP_DISCARD:
				return rpt_len;
			case CK_DUP_REPLACE:
			...
			case CK_DUP_NEXT_CK:
				/* Not a duplicate in that checksum chain
