 ...
 *
 * Rhyolite Software DCC 1.3.154-1.80 $Revision$
 */
...
wf_fnm_lno(DCC_FNM_LNO_BUF *buf, const WF *wf)
{
	...
	fno = wf->fno;
	if (!fno)
		return dcc_fnm_lno(buf, wf->ascii_nm, wf->lno);

	snprintf(buf->b, sizeof(buf->b),
		 DCC_FNM_LNO_PAT" included in %s",
		 wf->lno, dcc_path2fnm(wf->wtbl->hdr.white_incs[fno-1].nm),
		 wf->ascii_nm);
	return buf->b;
}
...
next_line:;
		...
		if (nmp) {
			...
			if (!dcc_fnm2rel(wf->wtbl->hdr.white_incs[white_fno].nm,
					 nm, 0)) {
				dcc_pemsg(EX_DATAERR, emsg,
					  "name \"%s\" too long%s",
