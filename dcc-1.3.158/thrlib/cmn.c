 ...
 *
 * Rhyolite Software DCC 1.3.158-1.179 $Revision$
 */
...
log2_start(CMN_WORK *cwp)
{
	...
	if (fsync(cwp->log_fd) < 0)
		thr_error_msg(cwp, "log_fd fsync(%s): %s",
			      dcc_fnm2abs_msg(abs_nm, cwp->log_nm),
			      ERROR_STR());
...
find_cwf(RCPT_ST *rcpt_st)
{
	...
	if (!dcc_fnm2rel(white_nm_buf, rcpt_st->dir, "/whiteclnt")) {
	...
	white_nm_ptr = dcc_path2fnm(white_nm_buf);
	       ...
	       RCPT_FGS global_fgs)
{
	...
	if (rcpt_st->fgs & (RCPT_FG_WHITE | RCPT_FG_BLACK))
		thr_log_print(cwp, 0, "%s%s\n",
			      ...
			      : cwf ? dcc_fnm2abs_msg(fnm, cwf->wf.ascii_nm)
			      : "",
...
user_log_write(RCPT_ST *rcpt_st, const void *buf, u_int len)
{
	...
	if (result < 0)
		thr_error_msg(rcpt_st->cwp, "write(%s): %s",
			      dcc_fnm2abs_msg(abs_nm, rcpt_st->user_log_nm),
			      ERROR_STR());
	else
		thr_error_msg(rcpt_st->cwp,
