 ...
 *
 * Rhyolite Software DCC 1.3.154-1.236 $Revision$
 */
...
{
#ifdef DCC_WIN32
	dcc_win32_unmap(&info_map, dcc_clnt_info, dcc_info_nm);
#else
		  ...
		  DCC_PATH new_info_nm, const char *vstr)
{
	...
	if (!dcc_fnm2rel(new_info_nm, dcc_info_nm, "-new")) {
		dcc_pemsg(EX_IOERR, emsg, "long map name \"%s\"",
			  dcc_info_nm);
		dcc_info_unlock(0);
		return -1;
		...
		DCC_INFO_FGS info_fgs)	/* DCC_INFO_FG_* */
{
	 ...
	 * rename()) open files */
	if (!dcc_fnm2rel(old_info_nm, dcc_info_nm, "-old")) {
		dcc_pemsg(EX_IOERR, emsg, "long map name \"%s\"",
			  dcc_info_nm);
		return -1;
	}
...
map_convert(DCC_EMSG emsg)
{
	...
	dcc_pemsg(EX_DATAERR, emsg, "%s is not a DCC map file", dcc_info_nm);
	close(info_fd);
	info_fd = -1;
	return 0;
}
	     ...
	     const char *new_info_nm, int new_info_fd)
{
	...
	for (;;) {
		...
		if (!dcc_ck_private(emsg, &dcc_info_sb, dcc_info_nm, info_fd)) {
			dcc_unmap_close_info(0);
			return 0;
		}
...
#ifdef DCC_WIN32
		dcc_clnt_info = dcc_win32_map(emsg, &info_map, dcc_info_nm,
	  ...
	  u_char complain)
{
	...
	if (complain && (!emsg || *emsg == '\0' || dcc_clnt_debug)) {
	 ...
	 sa_family_t family)
{
	...
	retries = -1;
	if (udp_create(emsg, &soc->s, &su, &retries)) {
		if (src)
			src->failed = 0;
		return 1;
	} else {
...
static void
clear_error(DCC_CLNT_CTXT_SOC *soc, const char *which)
{
	int err;
	socklen_t errlen;
	    ...
	    DCC_OP_RESP *resp, int resp_max_len)
{
	...
	do {
		...
		if (!get_now(emsg, ctxt))
			return 0;	/* give up if time jumped */
